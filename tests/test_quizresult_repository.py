    ...
    with patch.object(mock_db_session, "add", new_callable=AsyncMock) as mock_add:
        with patch.object(mock_db_session, "commit", new_callable=AsyncMock) as mock_commit:
            with patch.object(mock_db_session, "refresh", new_callable=AsyncMock) as mock_refresh:
                ...
                created_quiz_result = await quiz_result_repo.create_quiz_result(db=mock_db_session, **quiz_result_data)
                ...
                assert created_quiz_result.correct_answers == quiz_result_data["correct_answers"]
                assert created_quiz_result.total_questions == quiz_result_data["total_questions"]
...
async def test_calculate_user_average_score_in_company(mock_db_session):
    ...
    with patch.object(mock_db_session, "execute", new_callable=AsyncMock, return_value=mock_result):
        ...
        assert average_score["total_correct_answers"] == result_data["total_correct_answers"]
...
async def test_calculate_user_average_score_systemwide(mock_db_session):
    ...
    with patch.object(mock_db_session, "execute", new_callable=AsyncMock, return_value=mock_result):
        quiz_result_repo = QuizResultRepository()
        average_score = await quiz_result_repo.calculate_user_average_score_systemwide(db=mock_db_session, user_id=1)
