...
from app.schemas.quizzes import QuizCreate, Quiz, QuizRunResponse, QuizResult, QuizWithQuestions, \
    QuestionBase
...
async def get_quizzes_endpoint(
    ...
    quizzes = await quiz_repository.get_quizzes(db=db, company_id=company_id, user_id=current_user.id, skip=skip,
                                                limit=limit)
    return quizzes
...
@router.post("/{quiz_id}/run", response_model=QuizResult)
async def run_quiz_endpoint(
    ...
    await quiz_repository.check_if_member(db, user_id=current_user.id, quiz_id=quiz_id)

    result = await quiz_repository.run_quiz(db=db, quiz_id=quiz_id, responses=responses)
    return result
