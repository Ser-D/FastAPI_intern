...
from app.schemas.analytics import UserAverageScore, QuizAverageScore, QuizCompletion, MemberAverageScore, \
    MemberQuizCompletion, WeeklyMemberAverageScore, WeeklyScores, UserQuizWeeklyScores
...
@router.get("/company/{company_id}/user/{user_id}/quiz-scores-over-time", response_model=List[UserQuizWeeklyScores])
async def get_user_quiz_scores_over_time(
    ...
    # Перевірка, чи є користувач членом компанії
    await analytics_service.is_user_member_of_company(db, user_id, company_id)

    # Отримання результатів квізів за тижнями
