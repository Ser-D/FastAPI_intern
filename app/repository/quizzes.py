...
from app.schemas.quizzes import QuizCreate, QuizUpdate, QuizRunResponse, QuizResult
...
class QuizRepository:
    async def check_if_admin(self, db: AsyncSession, user_id: int, company_id: int) -> None:
        admin_check = await db.execute(
            select(Member).where(Member.company_id == company_id, Member.user_id == user_id, Member.is_admin == True)
        )
        admin = admin_check.scalar_one_or_none()
        if not admin:
            raise HTTPException(status_code=403, detail="Not authorized to perform this action")

    async def check_if_member(self, db: AsyncSession, user_id: int, quiz_id: int) -> None:
        ...
        if not member:
            raise HTTPException(status_code=403, detail="Not authorized - User is not a member of the company")
    ...
    async def create_quiz(self, db: AsyncSession, quiz: QuizCreate, company_id: int, user_id: int) -> QuizModel:
        ...
        if not all(q.company_id == company_id for q in questions):
            raise HTTPException(status_code=400, detail="All questions must belong to the same company as the quiz.")
    ...
    async def get_quizzes(self, db: AsyncSession, company_id: int, user_id: int, skip: int = 0, limit: int = 10) -> \
            List[QuizModel]:
        await self.check_if_admin(db, user_id=user_id, company_id=company_id)
        result = await db.execute(select(QuizModel).where(QuizModel.company_id == company_id).offset(skip).limit(limit))
        return result.scalars().all()

    async def update_quiz(self, db: AsyncSession, quiz_id: int, quiz: QuizUpdate, company_id: int,
                          user_id: int) -> QuizModel:
        await self.check_if_admin(db, user_id=user_id, company_id=company_id)
    ...
    async def run_quiz(self, db: AsyncSession, quiz_id: int, responses: List[QuizRunResponse]) -> QuizResult:
        db_quiz = await db.get(QuizModel, quiz_id)
        if not db_quiz:
            raise HTTPException(status_code=404, detail="Quiz not found")
        ...
        correct_answers_list = [sorted([str(answer) for answer in question.correct_answers]) for question in
                                db_questions]

        user_answers_list = [sorted([str(answer) for answer in response.selected_answers]) for response in responses]

        print(f"Correct Answers List: {correct_answers_list}")  # TODO: Remove
        print(f"User Answers List: {user_answers_list}")  # TODO: Remove
        ...
        incorrect_answers = len(correct_answers_list) - correct_answers_count
        score = (correct_answers_count / len(correct_answers_list)) * 100

        db_quiz.usage_count += 1
        await db.commit()
