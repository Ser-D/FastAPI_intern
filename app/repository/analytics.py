...
from app.models import QuizResult, Member
from typing import List, Optional, Tuple, Dict
from datetime import datetime, timedelta
from app.schemas.analytics import QuizAverageScore, QuizCompletion, MemberAverageScore, MemberQuizCompletion, \
    WeeklyScore, WeeklyMemberAverageScore, WeeklyScores, UserWeeklyScore, UserQuizWeeklyScores, QuizWeeklyScore
...
class AnalyticsService:
    ...
    async def get_user_quiz_scores(self, db: AsyncSession, user_id: int, start_date: Optional[datetime] = None,
                                   end_date: Optional[datetime] = None) -> List[QuizAverageScore]:
        query = select(
            QuizResult.quiz_id,
    ...
    async def get_user_quiz_completions(self, db: AsyncSession, user_id: int) -> List[QuizCompletion]:
        result = await db.execute(
            select(QuizResult.quiz_id, func.max(QuizResult.completed_at).label("last_completed"))
            .where(QuizResult.user_id == user_id)
    ...
    async def get_company_weekly_average_scores(self, db: AsyncSession, company_id: int) -> List[WeeklyScores]:
        # Отримання результатів квізів
        quiz_results = await self.get_company_quiz_results(db, company_id)

        if not quiz_results:
            return []

        # Визначення мінімальної та максимальної дати
        ...
        # Розбивка на тижні, починаючи з понеділка
        ...
        # Групування результатів по тижнях та користувачах
        ...
        # Обчислення середніх балів для кожного користувача за кожен тиждень
        weekly_averages = []
        ...
        # Повернення словника з групованими результатами
        return weekly_dict

    async def is_user_member_of_company(self, db: AsyncSession, user_id: int, company_id: int):
        result = await db.execute(
            select(Member).where(Member.user_id == user_id, Member.company_id == company_id)
        )
    ...
    async def get_user_quiz_scores_over_time(self, db: AsyncSession, user_id: int, company_id: int) -> List[
        UserQuizWeeklyScores]:
        # Отримання результатів квізів для вибраного користувача та компанії
        ...
        # Групування результатів по тижнях та квізах
        ...
        # Обчислення середніх балів для кожного квізу за кожен тиждень
        weekly_averages = []
    ...
    async def get_company_quiz_completions(self, db: AsyncSession, company_id: int) -> List[MemberQuizCompletion]:
        result = await db.execute(
            select(Member.user_id, func.max(QuizResult.completed_at).label("last_completed"))
            .join(QuizResult, QuizResult.user_id == Member.user_id)
            .where(Member.company_id == company_id)
