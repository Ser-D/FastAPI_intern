...
class NotificationRepository:
    async def create_quiz_notification(self, db: AsyncSession, company_id: int, quiz_id: int):
        users = await db.execute(select(User).join(Member).filter(Member.company_id == company_id))
    ...
    async def create_incomplete_quiz_notification(self, db: AsyncSession, user_id: int, quiz_id: int):
    ...
    async def get_user_notifications(self, db: AsyncSession, user_id: int):
        notifications = await db.execute(select(Notification).filter(Notification.user_id == user_id))
        return notifications.scalars().all()

    async def mark_notification_as_read(self, db: AsyncSession, notification_id: int, user_id: int):
